...
**********************************************************************
*/
static int g_iFdLcd=-1;
static char *g_pLcdMem=NULL;
#define LCD_MEM_SIZE (LCD_XSIZE*LCD_YSIZE*4)
#define LCD_LINE_SIZE (LCD_XSIZE*4)
...
void LCD_L0_SetPixelIndex(int x, int y, int PixelIndex) {
  ...
  {
  	char *ptr = g_pLcdMem + y*LCD_LINE_SIZE + x*4;
  	unsigned char r,g,b;

  	r = PixelIndex&0xff;
  	g = (PixelIndex>>8)&0xff;
  	b = (PixelIndex>>16)&0xff;
...
unsigned int LCD_L0_GetPixelIndex(int x, int y) {
  ...
  {
    char *ptr = g_pLcdMem + y*LCD_LINE_SIZE + x*LCD_BITSPERPIXEL;
  	unsigned char r,g,b;
  	b = *(ptr);
  	g = *(ptr+1);
  	r = *(ptr+2);
  	PixelIndex = (r<<16) | (g<<8) | b;
  }
  return PixelIndex;
}
...
void LCD_L0_FillRect(int x0, int y0, int x1, int y1) {
  ...
  for (j=y0; j<y1; j++)
  	for (i=x0; i<x1; i++){
		LCD_L0_SetPixelIndex(i, j, GUI_Context.BkColor );
  	}
}
...
int  LCD_L0_Init(void) {
  //LCD_INIT_CONTROLLER();
  g_iFdLcd =  open(LCD_FILE_PATH,O_CREAT|O_RDWR,00777);
	
	g_pLcdMem = mmap(NULL, LCD_MEM_SIZE, PROT_READ|PROT_WRITE,
       MAP_SHARED,g_iFdLcd,0);
       
  memset(g_pLcdMem, 0xff, LCD_MEM_SIZE);
